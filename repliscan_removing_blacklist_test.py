...
Methods to handle replicates:
  ...
  - max''')
	...
	#####################################################
	run_logFC(fList, args.log, L, normVals,args.blacklist)
	#####################################################
	# Apply Haar Wavelet
	#####################################################
...
def run_logFC(fList, useLog, L, normVals, blacklist):# thresh=2.5):
	if useLog:
	...
	for bIndex in xrange(1,len(beds)):
		outName = fList[bIndex][1]+fSuff
			...
			OF.close()
                if blacklist :
                        os.system("bedtools subtract -a "+outName+" -b G1_norm_blacklist.bedgraph > temp.bedgraph")
                        os.system("mv temp.bedgraph "+outName)
